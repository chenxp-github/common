// xImagePNG.cpp: implementation of the CxImagePNG class.
//
//////////////////////////////////////////////////////////////////////
#include "ximagepng.h"
#include "syslog.h"
...
{
    this->StartUp(CXIMAGE_FORMAT_PNG);
}
BOOL CxImagePNG::DecodeSizeOnly(CFileBase * hFile)
{
    png_struct *png_ptr;
    png_info *info_ptr;
...
BOOL CxImagePNG::Decode(CFileBase *hFile)
{
    png_struct *png_ptr;
    png_info *info_ptr;
    BYTE *row_pointers=NULL;
    ...
    * the compiler header file version, so that we know if the application
        ...
        * set up your own error handlers in the png_create_read_struct() earlier. */
        ...
        if (info_ptr->num_palette>0){
            ...
            SetPaletteColor(1,85,85,85,0);
            SetPaletteColor(2,170,170,170,0);
            SetPaletteColor(3,255,255,255,0);
        ...
        NEW_ARRAY(row_pointers,BYTE,info_ptr->rowbytes + 8);
...
{
    CFileBase* hFile = (CFileBase*)png_get_io_ptr(png_ptr);
    
    if(hFile)
    {
        hFile->Read(data,length);       
    }
}
...
{
    strcpy((char*)png_ptr->error_ptr,error_msg);
    longjmp(png_ptr->jmpbuf, 1);
}

BOOL CxImagePNG::Encode(CFileBase *hFile)
{
    hFile->SetSize(0);
    ...
    * in case we are using dynamically linked libraries.  REQUIRED.
        ...
        */
        if (setjmp(png_ptr->jmpbuf)){
            ...
            if (info_ptr->palette) DEL_ARRAY(info_ptr->palette);
        ...
        if (GetPalette()){
            ...
            NEW_ARRAY(info_ptr->palette,png_color,nc);
        ...
        BYTE *row_pointers;
        NEW_ARRAY(row_pointers,BYTE,row_size);
            ...
            do  {
                ...
                if (AlphaIsValid()){
                    for (long ax=head.biWidth-1; ax>=0;ax--){
                        c = BlindGetPixelColor(ax,ay,TRUE);
