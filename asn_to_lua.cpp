...
static status_t asn_sequence_to_lua(ASN_TO_LUA_PARAM_DEF)
{
    ASSERT(obj_ptr);
...
static status_t asn_integer_to_lua(ASN_TO_LUA_PARAM_DEF)
{
    ASSERT(obj_ptr);
    long *p = (long*)obj_ptr;
    ASSERT(p);
...
static status_t asn_octet_string_to_lua(ASN_TO_LUA_PARAM_DEF)
{
    ASSERT(obj_ptr);
...
static status_t asn_ia5_string_to_lua(ASN_TO_LUA_PARAM_DEF)
{
    ASSERT(obj_ptr);
...
static status_t asn_enumerated_to_lua(ASN_TO_LUA_PARAM_DEF)
{
    ASSERT(obj_ptr);
    long *p = (long*)obj_ptr;
    LOCAL_MEM(mem);
...
static status_t asn_bit_string_to_lua(ASN_TO_LUA_PARAM_DEF)
{
    ASSERT(obj_ptr);
...
static status_t asn_sequence_of_to_lua(ASN_TO_LUA_PARAM_DEF)
{
    ASSERT(obj_ptr);
...
status_t asn_to_lua(ASN_TO_LUA_PARAM_DEF)
{
    ...
    {        
        asn_choice_to_lua(CALL_LIST);
    }
    else if(descriptor->IsAsnSequence())
    {
        asn_sequence_to_lua(CALL_LIST);
    }
    else if(descriptor->IsAsnInteger())
    {
        asn_integer_to_lua(CALL_LIST);
    }
    else if(descriptor->IsAsnOctetString())
    {
        asn_octet_string_to_lua(CALL_LIST);
    }
    else if(descriptor->IsAsnEnumerated())
    {
        asn_enumerated_to_lua(CALL_LIST);
    }
    else if(descriptor->IsAsnBitString())
    {
        asn_bit_string_to_lua(CALL_LIST);
    }
    else if(descriptor->IsAsnSequenceOf())
    {
        asn_sequence_of_to_lua(CALL_LIST);
    }
